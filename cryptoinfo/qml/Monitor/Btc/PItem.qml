...
Row {
    ...
    Item {
        ...
        Row {
            ...
            property color _textColor: (monitor_btc_model.total_tx_value > 0 && modelData.tx_value > monitor_btc_model.total_tx_value / monitor_btc_model.items_len_qml()) ? theme.priceUpFontColor : theme.priceDownFontColor
            property real _itemWidth: row.width / repeater.model.length

            Repeater {
                id: repeater

                model: !!modelData ? [index + 1, modelData.blocktime, modelData.tx_hash, modelData.from, modelData.to, modelData.tx_value.toFixed(2)] : []
