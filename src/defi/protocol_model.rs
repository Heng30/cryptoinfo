...
use crate::defi::data::{ProtocolItem, RawProtocolItem as RawItem};
use crate::defi::sort::{SortDir, ProtocolSortKey as SortKey};
use crate::utility;
...
impl Model {
    ...
    pub fn init_from_engine(engine: &mut QmlEngine, model: QObjectPinned<Model>) {
        ...
        qml_register_enum::<SortKey>(cstr!("DefiProtocolSortKey"), 1, 0, cstr!("DefiProtocolSortKey"));
    }
    ...
    // 生成一个新条目
    fn new(raw_item: &RawItem) -> ProtocolItem {
        return ProtocolItem {
    ...
    fn insert_rows(&mut self, row: usize, count: usize) -> bool {
        ...
        for i in 0..count {
            self.data.insert(row + i, ProtocolItem::default());
        }
        (self as &mut dyn QAbstractListModel).end_insert_rows();
        self.count_changed();
        true
