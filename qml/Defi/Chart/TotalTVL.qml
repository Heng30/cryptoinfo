...
Base.TimeLineSeries {
    ...
    function _update(count) {
        ...
        var firstItem = defi_total_tvl_model.get_item(0);
        var endItem = defi_total_tvl_model.get_item(count - 1);
        ...
        for (var i = 0; i < count; i++) {
            var item = defi_total_tvl_model.get_item(i);
    ...
    Component.onCompleted: {
        var count = defi_total_tvl_model.count;
        _update(count);
    }
